...
function GoogleCalendar(tokenPath) {
	let googleCalendar = {
		auth: null
	};
	...
	function loadSecret() {
		return new Promise((resolve, reject) => {
			File.readFileAsync(Path.dirname(tokenPath) + '/client_secret.json')
				.then((content) => {
					try {
		...
		return new Promise((resolve, reject) => {
			r1.question('Enter the code from that page here:', code => {
				...
				oauth2Client.getToken(code, (err, token) => {
					...
					File.writeFileAsync(tokenPath, JSON.stringify(token));
					resolve(oauth2Client);
				});
			});
		});
